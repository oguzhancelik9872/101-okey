...
class Validator {
  ...
  static canProcessTile(tile, targetMeld, indicator) {
    ...
    // ============================================
    if (type === 'run') {
      // Scenario A: Check if replacing an existing Okey in this run is valid (Okey Alma / Çalma)
      const jokerIndices = [];
      tiles.forEach((t, idx) => {
      ...
      for (const jIdx of jokerIndices) {
        ...
        candidateTiles[jIdx] = tile; // Replace the Okey with the played tile
      ...
      // Scenario B: Normal appending/extending the run without stealing Okey
